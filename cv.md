---
layout: default
title: Curriculum Vitae
---

[← Back to Home](/)

# Curriculum Vitae

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=DawidRozycki&show_icons=true&theme=default&hide_border=true)](https://github.com/DawidRozycki)

## Professional Summary

Backend developer and entrepreneur with [X] years of experience building scalable systems and leading technical initiatives. Specialized in [your key technologies], with a proven track record of delivering high-impact solutions across multiple industries. Currently co-founding Nexly, where I architect and build the entire backend infrastructure.

---

## Professional Experience

### Co-Founder & Backend Developer - Nexly
*[Start Date] - Present*

Leading the technical development of Nexly's platform as co-founder and backend developer.

**Key Responsibilities:**
- Architected and built the entire backend infrastructure from the ground up
- [Key responsibility 2]
- [Key responsibility 3]

**Major Achievements:**
- [Achievement 1 with metrics]
- [Achievement 2 with metrics]
- [Achievement 3 with metrics]

**Technologies:** [List technologies used]

---

### [Previous Role] - [Previous Company]
*[Date Range]*

[Brief description of the company and your role]

**Key Responsibilities:**
- [Responsibility 1]
- [Responsibility 2]
- [Responsibility 3]

**Major Achievements:**
- [Achievement 1 with metrics]
- [Achievement 2 with metrics]

**Technologies:** [List technologies used]

---

### [Another Previous Role] - [Another Company]
*[Date Range]*

[Brief description of the company and your role]

**Key Responsibilities:**
- [Responsibility 1]
- [Responsibility 2]

**Major Achievements:**
- [Achievement 1]
- [Achievement 2]

**Technologies:** [List technologies used]

---

## Technical Skills

### Backend Development
- **Languages:** [e.g., Python, Node.js, Go, Java]
- **Frameworks:** [e.g., Django, Express, FastAPI, Spring Boot]
- **APIs:** [e.g., REST, GraphQL, gRPC]

### Databases & Data
- **SQL:** [e.g., PostgreSQL, MySQL]
- **NoSQL:** [e.g., MongoDB, Redis, DynamoDB]
- **Data Processing:** [e.g., ETL, data pipelines]

### Infrastructure & DevOps
- **Cloud Platforms:** [e.g., AWS, GCP, Azure]
- **Containers:** [e.g., Docker, Kubernetes]
- **CI/CD:** [e.g., GitHub Actions, Jenkins, GitLab CI]
- **Monitoring:** [e.g., Prometheus, Grafana, DataDog]

### Other Skills
- **Architecture:** Microservices, Event-driven architecture, System design
- **Security:** Authentication, Authorization, API security
- **Performance:** Optimization, Caching, Load balancing
- **Methodologies:** Agile, Scrum, TDD

---

## Education

### [Degree] - [University Name]
*[Graduation Year]*

[Major/Field of study]

**Relevant Coursework:** [If applicable]

**Achievements:** [Academic honors, projects, etc.]

---

## Notable Projects

### [Project Name]
**Role:** [Your role]  
**Description:** [What the project was about]  
**Impact:** [The results or impact]  
**Technologies:** [Technologies used]

### [Project Name]
**Role:** [Your role]  
**Description:** [What the project was about]  
**Impact:** [The results or impact]  
**Technologies:** [Technologies used]

### [Project Name]
**Role:** [Your role]  
**Description:** [What the project was about]  
**Impact:** [The results or impact]  
**Technologies:** [Technologies used]

---

## Certifications & Awards

- [Certification or Award 1] - [Year]
- [Certification or Award 2] - [Year]
- [Certification or Award 3] - [Year]

---

## Languages

- **English:** [Proficiency level]
- **[Other Language]:** [Proficiency level]

---

## Interests & Activities

[Brief description of professional interests, open source contributions, speaking engagements, blog, etc.]

---

[← Back to Home](/)
